...
# Lexicon of common Sinhala words to standard Singlish spellings
lexicon = {
    'කොහොමද': 'kohomada',
    'ලංකාව': 'lankawa',
    'ලංකාවේ': 'lankawe',
    ...
    'අම්මා': 'amma',
    'නත්තල්': 'naththal',
    'භක්තිකයින්': 'bhakthikayin',
    'බෞද්ධයින්': 'bauddhayin',
    'නෙවෙයි': 'neweyi',
    'ඉන්නෙ': 'inne',
    'සුභ': 'subha',
    'මොකද': 'mokada',
    ...
    'දේශපේමී': 'deeshapemi',
    'ජනතාව': 'janathawa',
    'හැදින්වීම': 'hadinwiima',
    'හැඳින්වීම': 'handinwiima',
}

def phonetic_transliterate_word(text: str) -> str:
    """
    Fallback character-by-character phonetic transliteration.
    """
    if not text:
        return ""
...
# Regex to split Sinhala words from other characters
token_pattern = re.compile(r'([\u0d80-\u0dff]+|[^\u0d80-\u0dff]+)')

def transliterate(text: str) -> str:
    """
    Hybrid transliteration method: Dictionary lookup (lexicon) + G2P Fallback.
    """
    if not text:
        return ""
    ...
    for token in tokens:
        # Check if the token is a Sinhala word
        if token and '\u0d80' <= token[0] <= '\u0dff':
            # Check lexicon
            if token in lexicon:
                result.append(lexicon[token])
            else:
                # Fallback to phonetic parser
                result.append(phonetic_transliterate_word(token))
        else:
            result.append(token)
